    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
            <header className="bg-white p-4 border-b shadow-sm flex items-center sticky top-0 z-10">
                <Link to="/" className="mr-4 text-gray-600 hover:bg-gray-100 p-2 rounded-full transition"><ArrowLeft className="w-6 h-6" /></Link>
            ...
            <main className="flex-grow p-4 space-y-4 overflow-y-auto">
                {loading ? <div className="text-center p-10 text-gray-500">로딩 중...</div> : 
                 reports.length === 0 ? <div className="text-center p-10 text-gray-400">등록된 신고가 없습니다.</div> : (
                    reports.map((report) => (
                        // 🚨🚨🚨 [수정] Link로 감싸서 클릭 시 상세 화면으로 이동하며 데이터 전달
                        <Link 
                            to={`/report-board/${report.id}`} 
                            state={{ report: report }} // 상세 화면으로 데이터 전달
                            key={report.id} 
                        ...
                        >
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center space-x-2">
                                    <span className={`px-2 py-1 rounded-lg text-xs font-bold ${report.type === 'danger' ? 'bg-red-100 text-red-600' : report.type === 'warning' ? 'bg-yellow-100 text-yellow-600' : 'bg-green-100 text-green-600'}`}>
                            ...
                            <div className="flex items-center text-gray-500 text-sm mb-3">
                                <MapPin className="w-4 h-4 mr-1" /> {report.location} <span className="mx-2 text-gray-300">|</span> {report.writer}
                            </div>
                            <p className="text-gray-600 text-sm leading-relaxed mb-4 line-clamp-2">{report.content}</p>
                            <div className="flex items-center space-x-4 border-t pt-3">
                                <span className="flex items-center text-gray-500 text-sm"><ThumbsUp className="w-4 h-4 mr-1.5" /> 좋아요 {report.likes}</span>
                                <span className="flex items-center text-gray-500 text-sm"><MessageSquare className="w-4 h-4 mr-1.5" /> 댓글 {report.comments}</span>
                            </div>
                        </Link>
                    ))
                )}
            ...
            <button 
                ...
                className="fixed bottom-6 right-6 bg-blue-600 text-white p-4 rounded-full shadow-xl hover:bg-blue-700 transition-transform active:scale-95 z-20"
            >
